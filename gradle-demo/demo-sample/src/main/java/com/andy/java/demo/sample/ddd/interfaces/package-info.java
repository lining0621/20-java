 ...
 * - 表面,门面(Facade): Facade的用意在于为远程客户端提供粗粒度的调用接口，它的主要工作就是将一个用户请求委派给一个或多个Service进行处理,
 *   也就是我们常说的Controller。通常也会在这层做JSR303参数校验
 */
package com.andy.java.demo.sample.ddd.interfaces;